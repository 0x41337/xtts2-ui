...
def gen_voice(string, spk, speed, english, remove_trailing_dots):
...
def update_dropdown(_=None, selected_speaker=default_speaker_name):
    return gr.Dropdown(
        choices=update_speakers(), value=selected_speaker, label="Select Speaker"
    )
...
with gr.Blocks() as app:
    ...
    with gr.Row():
        with gr.Column():
            ...
            remove_dots_checkbox = gr.Checkbox(
                label="Remove trailing dots", value=False
            )
            ...
            refresh_button.click(
                fn=update_dropdown, inputs=[], outputs=speaker_dropdown
            )
